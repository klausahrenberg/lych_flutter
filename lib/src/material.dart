...
abstract class LRecordWidget<T extends Object> extends StatefulWidget {
  ...
  T record;
  LRecordWidget(this.record, [String url = "http://localhost:8080/api/"])
      : repo = new LRepository(url);
}
...
abstract class LDefaultPane<T extends Object> extends StatefulWidget {
  ...
  LDefaultPane([String url = "http://localhost:8080/api/"])
      : repo = LRepository(url);
}

abstract class LDefaultPaneState<T extends Object>
    extends State<LDefaultPane<T>> {
  @override
  void initState() {
    super.initState();
    _updateState();
...
const TextStyle _defaultHighlightStyle = TextStyle(
    fontWeight: FontWeight.bold, backgroundColor: Colors.yellowAccent);
...
class FTextSpan extends StatelessWidget {
  ...
  FTextSpan(this._text, this._highlight,
      [this._highlightStyle = _defaultHighlightStyle])
      : super();
  ...
  List<InlineSpan> _getFormattedText() {
    ...
    if (_highlight.length > 0) {
      ...
      while (index > -1) {
        ...
        result.add(TextSpan(
            text: s.substring(index, index + _highlight.length),
